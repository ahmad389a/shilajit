...
const AboutMoringa = () => {
  ...
  return (
    <Fragment>
      <SEO
        ...
        description="Opplev bedre helse med Moringa ! Få autentiske Moringa-produkter for mer energi og velvære. Handle nå!"
      />
  ...
  <p className="heading">{t("Rich in Nutrients")}:</p>
  ...
  <p className="heading">{t("A Natural Pharmacy for Health and Wellness")}:</p>
  ...
  <p className="heading">{t("Harmony with Nature")}:</p>
  ...
  <p className="heading">{t("A Modern Superfood Boosting Health and Nutrition")}:</p>
  ...
  <p className="heading">{t("Exploring the Science: Moringa Powder's Health Claims and Considerations")}:</p>
 ...
 <p className="headingM">{t("Process for making Moringa powder")}?</p>
...
<p style={{textAlign:'center',fontSize:'24px', fontWeight:'600px'}}> <span style={{color:'red'}}>{t("Warning")}: </span>{t("Be aware of the following before consuming Moringa")}</p>
<br/>

<p style={{textAlign:'center'}}>{t("Moringa is a natural dietary supplement that has been used in traditional medicine for centuries")}. {t("Before you begin using Moringa")}, {t("Ensure that you purchase Moringa from reputable sources that test the product for purity and quality")}.
...
<div className="row">
  ...
  <Card style={{height:'95%',  textAlign:'center', backgroundColor:'#78d23d'}}>
      ...
      <h2> <LiaChildSolid/></h2>
        ...
        <Card.Text>
        {t("Keep Moringa out of reach of children")}.<br/> {t("This product should not be used by persons under the age of 18")}
...
<p> <b>{t("Remember")}  !</b>{t("that Moringa is a dietary supplement and should not replace a balanced diet and a healthy lifestyle")} . {t("If you experience any unusual side effects or health issues after taking Moringa")},{t("consult a doctor immediately")} . {t("This warning is for informational purposes only and is not intended as medical advice")}.</p>
...
{t("it is essential to understand that it is not a substitute for medical treatment")}. {t("If you have specific health concerns or conditions")}, {t("it is advisable to consult with a healthcare professional for proper diagnosis and treatment")}. {t("Additionally")},{t("individual reactions to Moringa can differ")} , {t("so it's important to use it cautiously and in accordance with recommended guidelines")}. </p>
</div> */}
</div>
      </LayoutOne>
    </Fragment>
